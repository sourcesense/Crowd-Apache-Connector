...
sub isValidPrincipalToken {
	...
	my $validationFactor1 = SOAP::Data->name('ValidationFactor' =>
					\SOAP::Data->value(
						SOAP::Data->name('name' => "User-Agent")->type('string')->attr({xmlns => $XMLNS}),
...
Atlassian::Crowd -rev /revision/ Perl bindings for Atlassian Crowd SOAP API
...
isValidPrincipalToken($serverURL, $app_name, $appToken, $principalToken)
